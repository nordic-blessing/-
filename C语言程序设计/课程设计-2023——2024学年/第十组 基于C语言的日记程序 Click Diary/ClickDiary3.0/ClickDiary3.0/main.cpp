...
void ToRegister()
{
	...
	while (1)
	{
		...
		{
			setcolor(WHITE);
			InputBox(newUser->birthday.year, 5, "请输入年");
			if (strlen(newUser->birthday.year)>0)
			{
				strcpy(birthday_year_create->text, newUser->birthday.year);
			}
			else
			{
		...
		{
			setcolor(WHITE);
			InputBox(newUser->birthday.month, 3, "请输入月");
			if (strlen(newUser->birthday.month) > 0)
			{
				strcpy(birthday_month_create->text, newUser->birthday.month);
			}
			else
			{
		...
		if (isClickButton(birthday_day_create, m))
		{
			...
			InputBox(newUser->birthday.day, 3, "请输入日");
			if (strlen(newUser->birthday.day) > 0)
			{
				strcpy(birthday_day_create->text, newUser->birthday.day);
			}
			else
			{
			...
			if (strlen(newUser->birthday.year) == 0
				&& strlen(newUser->birthday.month) == 0
				&& strlen(newUser->birthday.day) == 0)
			{
...
void Me()
{
	...
	outtextxy(220, 160, "昵称：");
	outtextxy(430, 160, read_user.name);
	outtextxy(220, 210, "邮件");
	outtextxy(430, 210, read_user.email);
	outtextxy(220, 260, "电话号码");
	outtextxy(430, 260, read_user.telephone);
	outtextxy(220, 310, "生日");
	outtextxy(400, 310, read_user.birthday.year);
	outtextxy(450, 310, read_user.birthday.month);
	outtextxy(500, 310, read_user.birthday.day);
	outtextxy(220, 360, "日记篇数");
	outtextxy(430, 360, read_user.name);
	

	
	Button* editinfo = creatButton(400, 460, 140, 80, "编辑信息", themecolor, themecolor_1, "等线", 17, menu_gray, menu_gray);
	Button* exit = creatButton(770, 0, 30, 30,"!", BLACK, RGB(85, 85, 85), "宋体", 13,RGB(255, 255, 255), RGB(254, 225, 136));
	BeginBatchDraw();
	while (1)
	{
		drawButton(editinfo);
